-- =============================================================================
-- DietKu — REFERRAL / AFFILIATE (FULL STACK FOR ONE PASTE)
-- Supabase Dashboard → SQL → New query → Run this ENTIRE file once.
--
-- Requires: public.profiles + public.ai_scan_quota_bypass (already in project).
--
-- AFTER RUN:
--   UPDATE public.profiles SET app_role = 'admin' WHERE id = 'YOUR_USER_UUID';
--
-- If CREATE TRIGGER fails: try EXECUTE PROCEDURE instead of EXECUTE FUNCTION (old PG).
-- =============================================================================
...
CREATE TRIGGER trg_referral_codes_updated_at
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.touch_referral_codes_updated_at();

-- ---------------------------------------------------------------------------
-- C) is_app_admin + paid entitlement + scan bypass
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- E) create_my_referral_code (affiliate, 7 days, one per owner)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- F) admin_create_referral_code (admin only)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- G) Row Level Security (referral tables)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- H) Profiles: admins can read all profiles (admin UI / joins)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- J) View referral_codes_with_stats (admin list + pagination in app)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- K) admin_patch_referral_code (full row replace + audit)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- L) redeem_referral_code — rate limit + HAS_ACTIVE_* errors (FINAL)
--     Default: 24 logged attempts / 15 min / user (no log row when RATE_LIMITED).
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.redeem_referral_code(TEXT, JSONB) IS
  'Redeem referral; rate limited (see v_rate_max / v_rate_window in function body).';
...
-- =============================================================================
-- Done. Analytics queries: supabase/REFERRAL_ANALYTICS.sql
-- =============================================================================
